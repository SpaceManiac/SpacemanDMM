            ...
            match &decl.return_type {
                ProcReturnType::InputType(input_type) => {
                    if let Some(path) = input_type.to_typepath()
                        && let Some(ty) = self.objtree.find(path)
                    {
                        self.return_type
...
impl<'o, 's> AnalyzeProc<'o, 's> {
        ...
        if let Some(parent) = self.proc_ref.parent_proc() {
            ...
            if self.calls_parent {
                if !matches!(
                    ...
                    Some((false, _))
                ) && let Some((true, location)) = self.env.must_not_call_parent.get(parent)
                {
                    error(
                        self.proc_ref.location,
                ...
                match expr {
                    Expression::Base { term, follow } => {
                                ...
                                self.env.must_be_pure.get_self_or_parent(proc)
                        {
                            error(
                                location,
                                format!("call to pure proc {call} discards return value"),
                            )
        ...
        match static_ty {
            ...
            StaticType::Type(ty) => {
                error(
                    location,
                    format!(
                        "initial() called on var{}/{}, but it always returns null for types",
                        ty.pretty_path(),
                        name
                    ),
                )
    ...
    fn visit_call(
        ...
        for arg in args {
            ...
            match arg {
                Expression::AssignOp {
                    ...
                    if let Some(term) = lhs.as_term()
                        && let Some(name) = term.as_kwarg_key()
                    {
                        // Don't visit_expression the kwarg key.
                        any_kwargs_yet = true;
                        ...
                        if !proc.parameters.iter().any(|p| p.name == *name) {
                            ...
                            let mut error =
                                error(location, format!("bad keyword argument {name:?} to {proc}"));
            ...
            if proc.is_builtin()
                && proc.name() == "initial"
                && analysis.static_ty != StaticType::None
            {
                if let Expression::Base { term, follow } = arg {
                    if let Some(Spanned {
                        location,
                        elem: Follow::Field(_, ident),
                    }) = follow.last()
                    {
                        self.initial_null_error(
                            analysis.static_ty.clone(),
                            ident.to_string(),
                            *location,
                        );
                    } else {
                        let argument_analysis =
                            self.visit_term(term.location, &term.elem, None, local_vars);
                        if let Term::Ident(ident_name) = &term.elem {
                            self.initial_null_error(
                                argument_analysis.static_ty,
