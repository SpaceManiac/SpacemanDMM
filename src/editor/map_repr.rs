...
use dmi::IconCache;
use map_renderer::{DrawCall, RenderPop, Vertex};
    ...
    pub fn save(&self, merge_base: Option<&Map>) -> Map {
        ...
        for (z, level) in self.levels.iter().enumerate() {
            for (_, inst) in level.instances.keys_iter() {
                coords.entry((inst.x as usize, (self.size.1 - 1 - inst.y) as usize, z as usize)).or_default().push(&inst.pop);
            }
        }
        ...
        for (coord, pop_list) in coords.iter() {
            map.grid[(coord.2, coord.1, coord.0)] = *reverse_dictionary.entry(&pop_list).or_insert_with(|| {
...
impl AtomZ {
    fn prep_instance(&mut self, pops: &mut WeakKeyHashMap<Weak<Prefab>, RenderPop>, (x, y): (u32, u32), prefab: Arc<Prefab>) -> usize {
        let vertices = pops.get(&prefab).map_or_else(|| [Vertex::default(); 4], |rpop| rpop.instance((x, y)));
    ...
    fn remove_instance(&mut self, z: u32, idx: usize) -> RemovedInstance {
        ...
        RemovedInstance { z, old, replaced_with: None }
    }
}
...
impl<K, V> DualPool<K, V> {
    ...
    pub fn keys_iter<'a>(&'a self) -> impl Iterator<Item = (usize, &'a K)> + 'a {
        self.keys.iter().enumerate().filter(move |(i, _)| !self.freelist.contains(i))
    }
}
