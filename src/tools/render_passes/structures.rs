...
impl RenderPass for GravityGen {
    fn expand<'a>(&self, atom: &Atom<'a>, objtree: &'a ObjectTree, output: &mut Vec<Atom<'a>>) -> bool {
        ...
        for &(count, x, y) in &[(1, 1, -2), (2, 0, -2), (3, -1, -2), (4, 1, -1), (5, 0, -1), (6, -1, -1), (7, 1, 0), (9, -1, 0)] {
    ...
    fn overlays<'a>(&self, atom: &mut Atom<'a>, objtree: &'a ObjectTree, _underlays: &mut Vec<Atom<'a>>, overlays: &mut Vec<Atom<'a>>) {
        // energy overlay goes above the middle part
        if atom.istype("/obj/machinery/gravity_generator/part/") && atom.get_var("icon_state", objtree) == &Constant::string("on_5") {
