...
use super::ast::{Expression, Parameter, PathOp, Prefab, VarType};
use super::constants::Constant;
use super::docs::DocCollection;
use super::{Context, DMError, Location};
...
impl ObjectTree {
    ...
    fn assign_parent_types(&mut self, context: &Context) {
        for (path, &type_idx) in self.types.iter() {
            ...
            self.graph.node_weight_mut(type_idx).unwrap().parent_type = idx;
        }
    }
    ...
    fn register_var<'a, I>(&mut self, location: Location, parent: NodeIndex, mut prev: &'a str, mut rest: I, comment: DocCollection) -> Result<Option<&mut TypeVar>, DMError>
    where
    ...
    fn register_proc(&mut self, location: Location, parent: NodeIndex, name: &str, is_verb: Option<bool>, parameters: Vec<Parameter>) -> Result<(usize, &mut ProcValue), DMError> {
        ...
        if proc.declaration.is_none() {
            proc.declaration = is_verb.map(|is_verb| ProcDeclaration { location, is_verb });
        }
    ...
    pub fn add_entry<'a, I: Iterator<Item = &'a str>>(&mut self, location: Location, mut path: I, len: usize, comment: DocCollection) -> Result<(), DMError> {
    ...
    pub fn add_var<'a, I: Iterator<Item = &'a str>>(&mut self, location: Location, mut path: I, len: usize, expr: Expression, comment: DocCollection) -> Result<(), DMError> {
