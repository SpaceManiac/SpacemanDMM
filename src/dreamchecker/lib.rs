...
impl<'o> fmt::Display for StaticType<'o> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            StaticType::None => write!(f, "(none)")?,
            StaticType::Type(typeref) => write!(f, "{}", typeref)?,
            StaticType::List{list, keys: _} => write!(f, "{}", list)?,
        }
        Ok(())
    }
}
...
impl<'o> StaticType<'o> {
    ...
    fn is_subtype_of(&self, other: &StaticType<'o>) -> bool {
        match (self, other) {
